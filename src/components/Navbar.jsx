...
export default function Navbar() {
  ...
  return (
    <nav
      className="hidden md:flex sticky top-0 z-40 items-center justify-between px-5 sm:px-8 h-16 bar-frost bar-frost-top"
    >
        ...
        {links.map(({ to, label }) => {
          ...
          return (
            <Link
              key={to}
              to={hrefFor(to)}
              className="nav-link tab-label px-3.5 py-2 rounded-lg"
              style={{
                fontSize: 12.5, letterSpacing: '0.11em', fontWeight: active ? 600 : 500,
                color: active ? 'var(--n900)' : 'var(--n400)',
